...
Managing a workspace full of agents becomes practical: the Terminals card doubles as a manager, and an opt-in Auto-listen mode ends the "agent goes deaf after a minute" story by pressing Enter on `termivin recv` for you when a nudge is safe to type.
...
- **Auto-listen mode — a per-agent opt-in that keeps agents from going deaf.** The agent CLIs have no event loop between turns, so `termivin recv --wait 60` returns and the agent has to remember to call it again; in practice a few minutes in, they forget. Turn Auto-listen on for an agent (pane `⋯` menu → *🎧 Auto-listen for mail…* or the same item on the dashboard row menu, both behind a confirm because this is the one place we press Enter for you) and Termivin types `termivin recv --wait 60` **followed by Enter** into the pane whenever mail arrives, provided every guard passes: the terminal is an agent CLI, is running, is idle (not working, not at an approval prompt), has been quiet for at least 8 s, and hasn't been nudged in the last 30 s. Bursty mail collapses into a single nudge — five messages in 400 ms fire one recv, since the first one drains all five. Mail that arrives while the agent is busy waits: as soon as the terminal turns idle, the nudge fires. A small 🎧 badge on the pane header and dashboard row shows when Auto-listen is on; a bulk **🎧 Auto-listen on N** button in the Terminals card turns it on across every eligible agent at once.
