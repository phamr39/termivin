...
- **Pane ⋯ menu** — *Rename*, *Refresh view* (re-measures and repaints a terminal that has gone sluggish or is drawing garbage), *Clear scrollback* (drops the 8000-line history a long session accumulates, keeping the process alive), *Open folder* and *Open in VS Code* — both on the terminal's working directory. "Open in VS Code" uses the `code` command when it is on your PATH (in VS Code: *Shell Command: Install 'code' command in PATH*); on macOS, where a GUI app inherits none of the directories that command installs into, Termivin falls back to asking macOS to open the folder in VS Code directly, so it works with no setup at all.
...
- **Agent bus** — AI agents running in your workspaces can discover each other and talk, with zero configuration: a loopback HTTP server starts with the app and its credentials are injected into every terminal Termivin spawns, so `termivin register / who / send / recv` just work inside them. The 🔗 button on an agent's pane types a ready-made connect prompt — typed, not sent, so you can read it over (or edit it) and press Enter yourself. Turn on **🎧 Auto-listen** (per-terminal, pane's ⋯ menu) and Termivin will type `termivin recv --wait 60` **and press Enter** whenever mail arrives at that agent — provided the pane is running, idle, quiet, has no approval prompt waiting, and hasn't been nudged in the last 30 seconds — so an agent that forgot to poll picks its mail up automatically. Registrations, topics and message history persist — the network comes back intact after a restart. Local-only and authenticated (bound to `127.0.0.1`, per-launch bearer token, browser `Origin` requests refused, rate-limited). Design notes in [`docs/AGENT-BUS.md`](docs/AGENT-BUS.md).
...
- **Workspace dashboard = communication map + agent manager** — toggle to *Dashboard* to see how the workspace's agents talk: terminals as chips (status LED, role, unread-mail badge), topics as central bus bars, bus traffic as glowing traces with live signal pulses in hacker-terminal green. The **Terminals** card doubles as a manager for busy workspaces — a search box (matches name + role), status chips (Working / Idle / Approval / Off bus / Exited / Saved) and a per-row `⋯` menu with *Open in canvas, Rename, Push, Connect, Restart, Stop, Open folder, Remove*; a bulk-action strip at the bottom offers ⚡ Restore all saved, 📬 Push all with unread mail, 🔗 Connect all off-bus (each behind a confirm dialog). The **Awaiting reply** panel lists every question an agent hasn't answered yet plus any agent with unread bus mail in its queue — each row has a 📬 *Push* button that types `termivin recv --wait 60` into the target agent's pane so you can nudge a forgetful agent without leaving the dashboard. The side panel also manages the bus itself: counts, per-terminal CPU/RAM, topics (create / delete / pick the representative), token usage for the workspace's folders, and a live activity feed.
...
- **Themes** — ⚙ in the sidebar footer, sixteen options: dark ones — Termivin (default), Matrix, Amber CRT, Ice Terminal, Synthwave, Bubblegum, Sakura, Nord, Dracula, Gruvbox, Tokyo Night, Sunset, Rose Pine — and two light ones — Cotton Candy (powder pink pastel) and Latte (warm cream, muted mauve). A theme restyles everything: panels, communication maps, and the terminals themselves (live xterm instances repaint on switch). Choice sticks across restarts.
